...
SELECT
	SUM(revenue.worldwide_gross) AS total_worldwide_gross, 
	ROUND(AVG(rating.imdb_rating),2) AS avg_rating,
	(specs.release_year/10) *10 AS decade
FROM revenue
JOIN rating 
	USING (movie_id)
JOIN specs
...
-----------------------------------------------------------------------------------------------------
-- 2.	Our goal in this question is to compare the worldwide gross for movies compared to 
--their sequels.  
...
*/
-----------------------------------------------------------------------------------------------------
-- b.	For each of these movies, create a new column showing the original film’s name by
--removing the last two characters of the film title. For example, for the film “Cars 2”,
--the original title would be “Cars”. Hint: You may find the string functions listed in Table 9-10 
...
-----------------------------------------------------------------------------------------------------
-- c.	Bonus: This method will not work for movies like “Harry Potter and the Deathly Hallows: 
...
SELECT
	film_title AS sequel,
CASE WHEN film_title = '% 2' THEN LEFT(film_title, -2),
CASE WHEN film_title = '% Part 2' THEN REPLACE(film_title, '%Part 2', '%Part 1')
END AS original
FROM specs
...
--Hint: You will likely need to perform a self-join on the specs table in order to get the movie_id 
--values for both the original films and their sequels. Bonus: A common data entry problem is 
--trailing whitespace. In this dataset, it shows up in the film_title field, where the movie “Deadpool”
--is recorded as “Deadpool “. One way to fix this problem is to use the TRIM function. 
...
-- a.	Write a query which, for each film will extract the portion of the film name that
--occurs before the colon. For example, “Transformers: Dark of the Moon” should result in “Transformers”.
--If the film title does not contain a colon, it should return the full film name.
--For example, “Transformers” should result in “Transformers”. Your query should return two columns, 
--the film_title and the extracted value in a column named series.
...
*/
-----------------------------------------------------------------------------------------------------
-- b.	Keep only rows which actually belong to a series. Your results should not include “Shark Tale”
--but should include both “Transformers” and “Transformers: Dark of the Moon”.
--Hint: to accomplish this task, you could use a WHERE clause which checks whether the 
--film title either contains a colon or is in the list of series values for films that do contain 
--a colon.  
/*
SELECT
    film_title,
...
--How many contain it twice? three times? four times? Hint: Look at the sting functions and 
--operators here: https://www.postgresql.org/docs/current/functions-string.html
/*
SELECT
    film_title,
...
*/
-----------------------------------------------------------------------------------------------------
-- 5.	For each distributor, find its highest rated movie. Report the company name, the film title, 
--and the imdb rating. Hint: you may find the LATERAL keyword useful for this question. 
--This keyword allows you to join two or more tables together and to reference columns provided 
--by preceding FROM items in later items. See this article for examples of lateral joins in 
--postgres: https://www.cybertec-postgresql.com/en/understanding-lateral-joins-in-postgresql/
...
-----------------------------------------------------------------------------------------------------
-- 6.	Follow-up: Another way to answer 5 is to use DISTINCT ON so that your query returns 
--only one row per company. You can read about DISTINCT ON on this page: 
